...
class UserController extends GetxController {
  ...
  createUserDetails(String shippingAddress, String paymentMethod, String phoneNumber, bool isAdmin) async {
    if (kDebugMode) {
      print("user_controller.dart - createUserDetails()");
    }
    DocumentSnapshot docSnapShot = await usersRef.doc(currentUser?.email).collection("UserDetails").doc(currentUser?.uid).get();
    if (!docSnapShot.exists) {
      if (kDebugMode) {
        print("createUserDetails() - User details not found, creating new user details...");
      }
      await usersRef.doc(currentUser?.email).collection("UserDetails").doc(currentUser?.uid).set({
        "shippingAddress": shippingAddress,
        "paymentMethod": paymentMethod,
        "phoneNumber": phoneNumber,
        "isAdmin": isAdmin,
      });
      docSnapShot = await usersRef.doc(currentUser?.uid).collection("UserDetails").doc(currentUser?.uid).get();
      if (kDebugMode) {
        print("user_controller.dart - New User Details Added: " + currentUser!.email!);
      }
      Get.to(() => const Home());
    } else {
      if (kDebugMode) {
